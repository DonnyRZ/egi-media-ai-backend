...
test("S12 Executive Summary is backend Top 20 while search exposes all issues", async () => {
  const server = await listen({ summary: { getExecutiveSummary: async () => ({ period: "24jam", items: Array.from({ length: 20 }, (_, i) => ({ issueId: `top-${i + 1}` })) }) }, issues: { list: async () => ({ items: Array.from({ length: 12 }, (_, i) => ({ issue_id: `issue-${i + 1}` })), page: 1, limit: 20, total: 12 }), detail: async ({ issueId }) => ({ issue_id: issueId, articles: [], developments: [] }) } });
  ...
  try {
    ...
    assert.equal(summary.status, 200); assert.equal(summaryBody.data.issues.length, 20); assert.equal(summaryBody.data.top5_limit, 20); assert.equal(search.status, 200); assert.equal(searchBody.data.meta.total, 12); assert.equal(detail.status, 200); assert.equal(detailBody.data.issue_id, "issue-12");
