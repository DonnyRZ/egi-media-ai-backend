...
function createT13ReportNarrativeRuntime({ aiTaskKernel, openaiConfig, reportDraftStore, authorizeCompany, promptRegistry, runStore, narrativeStore }) {
  const registry = promptRegistry || new PromptRegistry([createT13PromptDefinition({ modelName: openaiConfig.miniModel })]);
  const provenanceStore = runStore || new InMemoryPromptRunStore(); const narratives = narrativeStore || new InMemoryReportNarrativeStore();
  ...
  return { service: new ReportNarrativeService({ reportDraftStore, narrativeStore: narratives, promptExecutionService, authorizeCompany }), promptRegistry: registry, runStore: provenanceStore, narrativeStore: narratives };
}
