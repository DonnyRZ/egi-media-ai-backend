...
function buildSystemPolicy(context) {
  return [
    ...
    "For every subject_relation, explain what the evidence means for your company — never write an operations brief for the external company in the article.",
    "For competitor or market evidence: describe the external move factually, then frame why_matters, impacts, risks, and watch as strategic implications for your company.",
    "Do not claim your company owns a property, serves a segment, operates in a location, or has a capability unless that fact appears in company_context_fields.",
...
function buildT07Input({ tenantId, companyId, issue, context, evidence, outputLanguage, subjectRelation }) {
  ...
  const taskContract = {
    ...
    management_perspective_rule: [
      ...
      "impacts and risks: consequences for your company, not operational consequences for the external article subject.",
      "watch: indicators and response options your leadership can monitor or consider; never instructions for the external entity.",
      "If context is insufficient for a specific claim, state the uncertainty and propose what you should verify. Never invent company facts.",
    ],
...
module.exports = { SYSTEM_POLICY, buildT07Input, buildSystemPolicy };
