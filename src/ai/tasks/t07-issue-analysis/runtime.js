...
const { createT07PromptDefinition } = require("./definition");
const { IssueAnalysisService } = require("./service");
const { InMemoryIssueAnalysisStore } = require("./analysis.store");

function createT07IssueAnalysisRuntime({ aiTaskKernel, openaiConfig, cmsSourceGate, issueStore, getEffectiveContext, authorizeCompany, promptRegistry, runStore, analysisStore }) {
  const registry = promptRegistry || new PromptRegistry([createT07PromptDefinition({ modelName: openaiConfig.miniModel })]);
  ...
  return { service: new IssueAnalysisService({ cmsSourceGate, issueStore, getEffectiveContext, analysisStore: analyses, promptExecutionService, authorizeCompany }), promptRegistry: registry, runStore: provenanceStore, analysisStore: analyses };
}
