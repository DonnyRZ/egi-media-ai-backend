...
function createCompanyRouter({ getCompanyStore } = {}) {
  ...
  router.get("/api/v1/companies", scope, async (req, res, next) => {
    try {
    const role = req.authContext.role || req.authContext.membership?.role || req.authContext.actor?.role;
    let companies;
    if (isTenantOperator(role) && getCompanyStore?.()?.list) {
      const result = await getCompanyStore().list({ tenantId: req.authContext.tenantId, page: 1, limit: 100 });
      ...
      const userMemberships = typeof req.app.locals.membershipStore?.listForUser === "function"
        ...
        : (await req.app.locals.membershipStore?.list?.({ tenantId: req.authContext.tenantId, page: 1, limit: 100 }))?.items?.filter((item) => item.userId === req.authContext.actor.actorId) || [];
      const fromMemberships = userMemberships.filter((item) => item.tenantId === req.authContext.tenantId && item.companyId).map((item) => ({
        company_id: item.companyId,
        tenant_id: item.tenantId || req.authContext.tenantId,
        role: item.role,
      }));
      const claims = req.authContext.authorizedCompanies;
      companies = fromMemberships.length
        ? fromMemberships
  ...
  router.get("/api/v1/tenant/companies", adminScope, async (req, res, next) => {
    try {
      const result = await getCompanyStore().list({
      ...
      return res.json({
        success: true,
        data: { items: result.items.map(serializeCompany), meta: { page: result.page, limit: result.limit, total: result.total } },
        meta: { request_id: getRequestId(req), correlation_id: getCorrelationId(req) },
      });
    } catch (error) { return next(error); }
  });
