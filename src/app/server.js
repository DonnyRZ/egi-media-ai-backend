...
class Server {
  constructor() {
    ...
    const accountStore = process.env.AI_PERSISTENCE_MODE === "postgres" ? {
      ...
      findAny: async (email) => { const result = await this.getDatabaseRuntime().ai.query("SELECT id,email,full_name,status,password_hash FROM ai.users WHERE email=$1", [email]); const row = result.rows[0]; return row ? { email: row.email, fullName: row.full_name, role: null, actorType: "human", passwordHash: row.password_hash } : null; },
      save: async ({ userId, email, fullName, passwordHash }) => { await this.getDatabaseRuntime().ai.query("INSERT INTO ai.users (id,email,full_name,status,password_hash) VALUES ($1,$2,$3,'active',$4) ON CONFLICT (email) DO UPDATE SET full_name=COALESCE(EXCLUDED.full_name,ai.users.full_name),password_hash=CASE WHEN ai.users.password_hash IS NULL OR btrim(ai.users.password_hash)='' THEN EXCLUDED.password_hash ELSE ai.users.password_hash END,status='active',updated_at=now()", [userId, email, fullName, passwordHash]); },
    } : null;
  ...
  _getIngestRuntime() {
    if (!this.ingestRuntime) {
      ...
      const crawlIngestService = new CrawlIngestService({ crawlArticleReader: this._getCrawlArticleReader(), sourceGate: this._getIssueSourceResolver(), snapshotStore, watermarkStore, enqueueStageJob, logger: this.logger, getTenantStore: () => this.tenantStore });
  ...
  _getPipelineRuntime() {
    ...
    registry.register("T02", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT02Service().classify({ tenantId, companyId, pipelineId, articleId: input.article_id, locale: input.locale }); return { nextInput: { decision_id: result.decision?.decisionId }, nextTaskId: result.shouldContinue ? "T03" : null, afterNextTaskId: result.shouldContinue ? "T04" : null, result }; });
    registry.register("T03", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT03Service().generate({ tenantId, companyId, pipelineId, decisionId: input.decision_id }); return { nextInput: { decision_id: result.decision?.decisionId }, nextTaskId: "T04", afterNextTaskId: "T05", result }; });
    registry.register("T04", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT04Service().match({ tenantId, companyId, pipelineId, relevanceDecisionId: input.decision_id }); const mutation = await this._getIssueMutationService().apply({ tenantId, companyId, matchDecisionId: result.match.matchDecisionId }); return { nextInput: { issue_id: mutation.mutation?.issueId || mutation.issueId }, nextTaskId: "T05", afterNextTaskId: "T06", result: { match: result, mutation } }; });
    registry.register("T05", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT05Service().generate({ tenantId, companyId, pipelineId, issueId: input.issue_id }); return { nextInput: { issue_id: result.issue?.issueId || input.issue_id }, nextTaskId: "T06", afterNextTaskId: "T07", result }; });
    registry.register("T06", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT06Service().generate({ tenantId, companyId, pipelineId, issueId: input.issue_id }); return { nextInput: { issue_id: result.issue?.issueId || input.issue_id }, nextTaskId: "T07", afterNextTaskId: "T08", result }; });
    registry.register("T07", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT07Service().analyze({ tenantId, companyId, pipelineId, issueId: input.issue_id }); return { nextInput: { issue_id: input.issue_id, analysis_id: result.analysis?.analysisId }, nextTaskId: "T08", afterNextTaskId: "T09", result }; });
    registry.register("T08", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT08Service().label({ tenantId, companyId, pipelineId, analysisId: input.analysis_id }); const promoted = await this._getCitationGate().validateAndPromote({ tenantId, companyId, analysisId: input.analysis_id }); return { nextInput: { issue_id: input.issue_id, analysis_id: input.analysis_id }, nextTaskId: "T09", afterNextTaskId: "T10", result: { labels: result, promoted } }; });
    registry.register("T09", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT09Service().evaluate({ tenantId, companyId, pipelineId, issueId: input.issue_id, analysisId: input.analysis_id }); return { nextInput: { issue_id: input.issue_id, analysis_id: input.analysis_id, priority_decision_id: result.priority?.priorityDecisionId }, nextTaskId: "T10", afterNextTaskId: null, result }; });
    ...
    registry.register("T10", async ({ tenantId, companyId, pipelineId, input }) => { const result = await this._getT10Service().generate({ tenantId, companyId, pipelineId, issueId: input.issue_id, analysisId: input.analysis_id, priorityDecisionId: input.priority_decision_id }); const downstream = await downstreamBoundary.evaluate({ tenantId, companyId, issueId: input.issue_id, pipelineId }); return { result: { priority_reason: result, downstream } }; });
    registry.register("T12", async ({ tenantId, companyId, pipelineId, input }) => ({ result: await this._getT12Service().generate({ tenantId, companyId, pipelineId, alertEventId: input.alert_event_id }) }));
