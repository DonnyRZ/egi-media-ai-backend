...
class InMemoryJobStore {
  ...
  list({ tenantId, companyId, status } = {}) { return [...this.jobsById.values()].filter((job) => (!tenantId || job.tenantId === tenantId) && (!companyId || job.companyId === companyId) && (!status || job.status === status)).map(clone); }
...
module.exports = { InMemoryJobStore, JOB_STATUSES, jobKey };
