...
class InMemoryCompanyStore {
  ...
  async create(input) { const value = normalizeCompany({ ...input, companyId: input.companyId || randomUUID(), status: input.status || "active" }); const scope = key(value.tenantId, value.companyId); if (this.companies.has(scope)) return { company: this.companies.get(scope), reused: true }; this.companies.set(scope, value); return { company: { ...value }, reused: false }; }
...
function normalizeCompany(input) { if (!input?.tenantId || !input?.companyId || !input?.name) throw new ProvisioningError("Tenant, company ID, and company name are required"); if (!TENANT_STATUSES.includes(input.status || "active")) throw new ProvisioningError("Company status is invalid"); return { companyId: input.companyId, tenantId: input.tenantId, name: String(input.name).trim(), legalName: input.legalName || null, status: input.status || "active", timezone: input.timezone || null, locale: input.locale || null, metadata: input.metadata || {}, createdAt: input.createdAt || new Date().toISOString(), updatedAt: input.updatedAt || new Date().toISOString() }; }
