...
class InMemoryMembershipStore {
  ...
  async list({ tenantId, companyId = undefined, page = 1, limit = 50 }) { const values = [...this.memberships.values()].filter((item) => item.tenantId === tenantId && (companyId === undefined || item.companyId === companyId)); const start = (page - 1) * limit; return { items: values.slice(start, start + limit).map((item) => ({ ...item, permissions: [...permissionsForRole(item.role)] })), page, limit, total: values.length }; }
  async get({ membershipId, tenantId, companyId = undefined }) {
    ...
    return item ? { ...item, permissions: [...permissionsForRole(item.role)] } : null;
  }
  async listForUser({ userId }) { return [...this.memberships.values()].filter((item) => item.userId === userId && item.status === "active").map((item) => ({ ...item, permissions: [...permissionsForRole(item.role)] })); }
  async invite({ userId, email, fullName, tenantId, companyId = null, role, idempotencyKey }) {
    ...
    this.users.set(resolvedUserId, { userId: resolvedUserId, email: normalizedEmail || email, fullName: fullName || null, status: existing?.status === "active" ? "active" : "invited" });
    return {
      membership: this.upsert({
        ...(existing || {}),
...
class PostgresMembershipStore {
  ...
  async list({ tenantId, companyId = undefined, page = 1, limit = 50 }) { const values = [tenantId]; const filters = ["tenant_id=$1"]; if (companyId !== undefined) { values.push(companyId); filters.push(`company_id=$${values.length}`); } values.push(limit, (page - 1) * limit); const result = await this.db.query(`SELECT id AS membership_id,user_id,tenant_id,company_id,role,status,version FROM ai.memberships WHERE ${filters.join(" AND ")} ORDER BY created_at DESC LIMIT $${values.length - 1} OFFSET $${values.length}`, values); return { items: result.rows.map((row) => ({ membershipId: row.membership_id, userId: row.user_id, tenantId: row.tenant_id, companyId: row.company_id, role: row.role, status: row.status, version: row.version, permissions: [...permissionsForRole(row.role)] })), page, limit, total: result.rowCount }; }
  async get({ membershipId, tenantId, companyId = undefined }) { const values = [membershipId, tenantId]; const companyFilter = companyId === undefined ? "" : " AND company_id=$3"; if (companyId !== undefined) values.push(companyId); const result = await this.db.query(`SELECT id AS membership_id,user_id,tenant_id,company_id,role,status,version FROM ai.memberships WHERE id=$1 AND tenant_id=$2${companyFilter} LIMIT 1`, values); const row = result.rows[0]; return row ? { membershipId: row.membership_id, userId: row.user_id, tenantId: row.tenant_id, companyId: row.company_id, role: row.role, status: row.status, version: row.version, permissions: [...permissionsForRole(row.role)] } : null; }
  async listForUser({ userId }) { const result = await this.db.query("SELECT id AS membership_id,user_id,tenant_id,company_id,role,status,version FROM ai.memberships WHERE user_id=$1 AND status='active' ORDER BY created_at ASC", [userId]); return result.rows.map((row) => ({ membershipId: row.membership_id, userId: row.user_id, tenantId: row.tenant_id, companyId: row.company_id, role: row.role, status: row.status, version: row.version, permissions: [...permissionsForRole(row.role)] })); }
  async invite({ userId, email, fullName, tenantId, companyId = null, role }) {
    const normalizedEmail = typeof email === "string" ? email.trim().toLowerCase() : null;
    ...
    const nextStatus = existing.rows[0]?.status === "active" ? "active" : "invited";
    await this.db.query(`INSERT INTO ai.users (id,email,full_name,status) VALUES ($1,$2,$3,$4) ON CONFLICT (email) DO UPDATE SET full_name=COALESCE(EXCLUDED.full_name,ai.users.full_name), updated_at=now()`, [resolvedUserId, normalizedEmail || `${resolvedUserId}@invalid.local`, fullName || null, nextStatus]);
    ...
    return { membership: { membershipId: row.membership_id, userId: row.user_id, tenantId: row.tenant_id, companyId: row.company_id, role: row.role, status: row.status, version: row.version, permissions: [...permissionsForRole(row.role)] }, reused: existing.rowCount > 0 };
  }
