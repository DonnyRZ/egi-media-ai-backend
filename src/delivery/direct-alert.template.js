...
function renderDirectAlertTemplate({ issue, blurb, detailUrl }) {
  const subject = `[EGI Media] Prioritas tinggi: ${sanitizeHeaderValue(issue.title)}`;
  const text = [
    `Isu: ${issue.title}`,
...
function sanitizeHeaderValue(value) { return String(value).replace(/[\u0000-\u001F\u007F]/g, " ").replace(/\s+/g, " ").trim(); }
