...
class IngestScheduler {
  ...
  start() { if (!this.config.enabled || this.timer) return false; this.running = true; this.tick(); this.timer = this.setTimer(() => this.tick(), this.config.intervalMs); return true; }
...
class MultiTenantIngestScheduler extends IngestScheduler {
  ...
  async tick() {
    ...
    const results = [];
    const companies = await this.listEligible();
