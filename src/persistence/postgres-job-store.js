...
class PostgresJobStore {
  ...
  async createOrGet({ tenantId, companyId, queueName, jobType, idempotencyKey, payload, maxAttempts = 3, availableAt = Date.now() }) {
    const id = this.uuid(); const result = await this.db.query("INSERT INTO ai.queue_jobs (id,tenant_id,company_id,queue_name,job_type,idempotency_key,payload_jsonb,max_attempts,available_at) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9) ON CONFLICT (tenant_id,company_id,queue_name,idempotency_key) DO NOTHING RETURNING *", [id,tenantId,companyId,queueName,jobType,idempotencyKey,JSON.stringify(payload),maxAttempts,new Date(availableAt).toISOString()]);
