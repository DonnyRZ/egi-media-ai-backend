...
class PostgresCompanyContextDraftStore {
  ...
  async listByCompany(companyId) { const rows = await this.db.query("SELECT * FROM ai.company_context_drafts WHERE company_id=$1 ORDER BY updated_at DESC", [companyId]); return rows.rows.map(mapDraft); }
...
class PostgresEffectiveCompanyContextStore {
  constructor({ db, uuid = randomUUID } = {}) { this.db = db; this.uuid = uuid; }
  async getEffective(companyId) { const row = await this.db.query("SELECT * FROM ai.company_contexts WHERE company_id=$1 AND status='effective' LIMIT 1", [companyId]); return row.rows[0] ? mapContext(row.rows[0]) : null; }
  async getVersion(companyId, version) { const row = await this.db.query("SELECT * FROM ai.company_contexts WHERE company_id=$1 AND version=$2 LIMIT 1", [companyId, version]); return row.rows[0] ? mapContext(row.rows[0]) : null; }
  async activate({ tenantId = "unknown", companyId, fields, source, actorId, draftId = null, changeReason = null, expectedNextVersion }) {
    const current = await this.db.query("SELECT COALESCE(MAX(version),0)::int AS version FROM ai.company_contexts WHERE company_id=$1", [companyId]); const nextVersion = current.rows[0].version + 1;
    ...
    await this.db.query("UPDATE ai.company_contexts SET status='archived',updated_at=now() WHERE company_id=$1 AND status='effective'", [companyId]);
    const id = this.uuid(); const now = new Date().toISOString(); const content = { fields, source, draftId, changeReason, updatedBy: actorId };
