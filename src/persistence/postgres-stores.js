...
class PostgresAlertEventStore {
  ...
  async listScoped({ tenantId, companyId, recipientId = null, page = 1, limit = 20 }) { const values=[tenantId,companyId]; let where="tenant_id=$1 AND company_id=$2"; if(recipientId){values.push(recipientId);where+=` AND recipient_ref=$${values.length}`;} const count=await this.db.query(`SELECT count(*)::int AS total FROM ai.alert_events WHERE ${where}`,values); values.push(limit,(page-1)*limit); const rows=await this.db.query(`SELECT * FROM ai.alert_events WHERE ${where} ORDER BY created_at DESC LIMIT $${values.length-1} OFFSET $${values.length}`,values); return {items:rows.rows.map(mapAlertEvent),page,limit,total:count.rows[0].total}; }
...
class PostgresReportDraftStore {
  constructor({ db, uuid = randomUUID } = {}) { this.db = db; this.uuid = uuid; }
  async createDraft(value) { const id=value.reportId && value.reportId !== "draft-validation" ? value.reportId : this.uuid(); const now=new Date().toISOString(); const payload={...value,reportId:id,version:1,reviewStatus:"draft",activity:[],createdAt:now,updatedAt:now}; const result=await this.db.query("INSERT INTO ai.reports (id,tenant_id,company_id,report_type,period_start,period_end,timezone,review_status,payload_jsonb) VALUES ($1,$2,$3,$4,$5,$6,$7,'draft',$8::jsonb) RETURNING *",[id,value.tenantId,value.companyId,value.reportType,value.periodStart,value.periodEnd,value.timezone,JSON.stringify(payload)]); return mapReport(result.rows[0]); }
  async get({ tenantId, companyId, reportId }) { const result=await this.db.query("SELECT * FROM ai.reports WHERE id=$1 AND tenant_id=$2 AND company_id=$3 LIMIT 1",[reportId,tenantId,companyId]); return result.rows[0] ? mapReport(result.rows[0]) : null; }
  async list({ tenantId, companyId, page=1, limit=20, reportType=null, reviewStatus=null }) { const values=[tenantId,companyId]; const predicates=["tenant_id=$1","company_id=$2"]; if(reportType){values.push(reportType);predicates.push(`report_type=$${values.length}`);} if(reviewStatus){values.push(reviewStatus);predicates.push(`review_status=$${values.length}`);} const where=predicates.join(" AND "); const count=await this.db.query(`SELECT count(*)::int AS total FROM ai.reports WHERE ${where}`,values); values.push(limit,(page-1)*limit); const rows=await this.db.query(`SELECT * FROM ai.reports WHERE ${where} ORDER BY updated_at DESC LIMIT $${values.length-1} OFFSET $${values.length}`,values); return {items:rows.rows.map(mapReport),page,limit,total:count.rows[0].total}; }
